...
filePath = '/Users/amc/Documents/Gait-Tracking-With-x-IMU-Gaitmap/datasets/new/Marcha_serie_03_29_pasos_20metros_20260428_122728_tem2.txt'
startTime = 0
stopTime = 10000

# Factor para compensar la distancia perdida por el límite de 4g del sensor al correr.
# En lugar de restar Hz (lo cual daña los filtros), ajusta este multiplicador manualmente:
# - 1.11 equivale a restar 5 Hz (para zancadas más cortas, ej. 21-22 pasos)
# - 1.38 equivale a restar 15 Hz (para zancadas largas/explosivas, ej. 17 pasos)
factor_compensacion_carrera = 1.11
...
def process_trajectory(imu_data, sampling_rate, tipo_actividad, umbral_zupt=25.0, type_sensor=1):
    ...
    if tipo_actividad == 'correr':
        ...
        # Detección de salto: buscar pico de aterrizaje en aceleración vertical (acc_ml)
        acc_vert = imu_data['acc_ml'].values
        peaks, _ = find_peaks(acc_vert, height=15.0, distance=sampling_rate*0.1)
        if len(peaks) == 0:
            inicio, fin = 0, len(imu_data)-1
        else:
            inicio = max(0, peaks[0] - 30)
            fin = min(len(imu_data), peaks[-1] + 20)
        stride_list = pd.DataFrame({'start': [inicio], 'end': [fin]})
        stride_list.index.name = 's_id'
        zero_velocity_events = {0: pd.DataFrame({'min_vel': [inicio, fin]})}
    ...
    if tipo_actividad == 'salto':
        # Integrador de posición (no da orientación)
        pos_integrator = PieceWiseLinearDedriftedIntegration()
        pos_result = pos_integrator.estimate(
            data=imu_xyz,
            stride_event_list=zero_velocity_events,
            sampling_rate_hz=sampling_rate
        )
        # Integrador de orientación (solo usa giróscopo)
        ...
        # Extraer posición
        if isinstance(pos_result.position_, dict):
            position_df = pd.concat(pos_result.position_.values()).reset_index(drop=True)
        else:
            position_df = pos_result.position_.reset_index(drop=True)
    ...
    # Fuerza Z no negativo (aplica a todas las actividades)
...
if actividad_actual in ['correr', 'salto'] or (actividad_actual == 'marcha' and freq_str == '100'):
    ...
    if not imu_df_2.empty:
        ...
        umbral = 21.9 if actividad_actual == 'correr' else 25.9
    ...
    # Ambos pies recorren la misma distancia total en la pista. Sin embargo, el Sensor 1
    # pierde distancia calculada porque su hardware satura a 4g más a menudo. 
    # Usamos el cálculo de distancia del Sensor 2 como referencia física y escalamos 
    # la trayectoria del Sensor 1 para igualarlo, sumándole los 10 cm (0.1 m) de ventaja.
    if actividad_actual == 'correr' and len(pos1_gm) > 1 and len(pos2_gm) > 1:
        ...
        # === COMPENSACIÓN GLOBAL DE DISTANCIA (20 METROS) ===
        # Aplicamos el multiplicador manual (definido al inicio del archivo) a AMBOS sensores
        # para que alcancen la distancia real de la pista (ej. 20m) juntos y sin desfasarse.
...
# =========================================================================
# ALINEACIÓN DE RUMBO Y DETRENDING AVANZADO (solo si no es salto)
# =========================================================================
...
fs = float(freq_str)
SamplePlotFreq = 2 if fs == 50.0 else 4
Spin = 120
