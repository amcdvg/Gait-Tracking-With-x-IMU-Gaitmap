...
# Factor para compensar la distancia perdida por el límite de 4g del sensor al correr.
factor_compensacion_carrera = 1.11#11
...
if filePath.endswith('.txt'):
    ...
    # Filter for canal == 'Mov' (Sensor 1 - Pie Derecho)
...
# -------------------------------------------------------------------------
# Detect stationary periods (for manual fallback and initial detection)
...
# =========================================================================
# CLASIFICADOR DE ACTIVIDAD (HIBRIDACIÓN CONDICIONAL)
# =========================================================================
...
elif acc_peak > 4.4 or energia_senal > 1.4:
...
# =========================================================================
# FUNCIONES UNIFICADAS PARA CARGA Y PROCESAMIENTO GAITMAP
# =========================================================================
...
N_BIAS_SAMPLES = 63#62#46#51#45#135#245#90#45

def load_imu_data(filepath, sensor_name):
    """Carga un archivo de datos de IMU y devuelve DataFrame con columnas
    ...
    # Manejo para archivos con dos IDs (50 Hz)
    if fs_val < 100.0:
...
def process_trajectory(imu_data, sampling_rate, tipo_actividad, umbral_zupt=25.0, type_sensor=1):
    """
    Procesa IMU data con gaitmap según tipo de actividad.
    Devuelve pos (N,3) y quats (N,4).
    """
    # --- Detección de eventos / regiones según actividad ---
    if tipo_actividad == 'correr':
        # =========== DETECCIÓN ORIGINAL (VALLES EN GIROSCOPIO) ===========
        gyr_mag = np.linalg.norm(imu_data[['gyr_pa', 'gyr_ml', 'gyr_si']].values, axis=1)
        nyq = 0.33 * sampling_rate
        b_filt, a_filt = butter(3, 3.0 / nyq, btype='low')
        gyr_mag_filt = filtfilt(b_filt, a_filt, gyr_mag)

        distance_samples = int(sampling_rate )#0.30)
        valles, _ = find_peaks(-gyr_mag_filt, distance=distance_samples, prominence=0.08)
        if len(valles) > 0:
            if valles[0] > 0:
                valles = np.insert(valles, 0, 0)
            if valles[-1] < len(gyr_mag_filt) - 1:
                valles = np.append(valles, len(gyr_mag_filt) - 1)
        else:
            valles = np.array([0, len(gyr_mag_filt) - 1])
        zero_velocity_events = {0: pd.DataFrame({'min_vel': valles})}

    elif tipo_actividad == 'salto':
        # --- 1. Pico de aterrizaje en aceleración vertical (acc_si) ---
        print(imu_data.keys())
        acc_vert = imu_data['acc_si'].values   # ahora en m/s² después de load_imu_data
        peaks, _ = find_peaks(acc_vert, height=25.0, distance=sampling_rate*2)
        print(peaks)
        if len(peaks) == 0:
            inicio, fin = 0, len(imu_data)-1
        else:
            inicio = max(0, peaks[0] - 30)
            fin = min(len(imu_data), peaks[-1] + 20)

        # --- 2. Detección de periodos estacionarios (mismo filtro que al inicio, pero con datos en m/s²) ---
        acc_mag = np.sqrt(imu_data['acc_pa']**2 + imu_data['acc_ml']**2 + imu_data['acc_si']**2)  # m/s²
        # Filtro paso alto (0.001 Hz)
        b_hp, a_hp = butter(1, (2.0*0.001) / sampling_rate, btype='high')
        acc_mag_filt = np.abs(filtfilt(b_hp, a_hp, acc_mag))
        # Filtro paso bajo (5 Hz)
        b_lp, a_lp = butter(1, (2.0*5.0) / sampling_rate, btype='low')
        acc_mag_filt = filtfilt(b_lp, a_lp, acc_mag_filt)

        # *** UMBRALES EN m/s² (0.6 g ≈ 5.886, 1.4 g ≈ 13.734, 0.2 g ≈ 1.962) ***
        stationary = (acc_mag_filt < 2.962) & (acc_mag > 5.886) & (acc_mag < 13.734)
        
        # --- 3. Construir lista de eventos de velocidad cero ---
        stat_events = []
        in_stat = False
        for i, is_stat in enumerate(stationary):
            if is_stat and not in_stat:
                stat_events.append(i)
                in_stat = True
            elif not is_stat and in_stat:
                stat_events.append(i)
                in_stat = False
        if in_stat:
            stat_events.append(len(stationary)-1)
       
        # Combinar con el intervalo del salto y ordenar
        all_events = sorted(set([inicio, fin] + stat_events))
        # Filtrar eventos muy cercanos (<10 muestras)
        filtered_events = [all_events[0]]
        for ev in all_events[1:]:
            if ev - filtered_events[-1] > 10:
                filtered_events.append(ev)
        print(filtered_events)
        zero_velocity_events = {0: pd.DataFrame({'min_vel': filtered_events})}
    ...
    # Convertir a formato gaitmap
    imu_xyz = imu_data.rename(columns={
        'acc_pa':'acc_x', 'acc_ml':'acc_y', 'acc_si':'acc_z',
        'gyr_pa':'gyr_x', 'gyr_ml':'gyr_y', 'gyr_si':'gyr_z'
    })

    # --- Estimación de posición y orientación según actividad ---
    if tipo_actividad == 'salto':
        # Integrador de posición (RtsKalman proporciona orientación si está disponible)
        pos_integrator = RtsKalman()
        result = pos_integrator.estimate(
            data=imu_xyz,
            stride_event_list=zero_velocity_events,
            sampling_rate_hz=sampling_rate
        )
        # Intentar obtener orientación del mismo resultado (mejor corregida)
        if hasattr(result, 'orientation_') and result.orientation_ is not None:
            if isinstance(result.orientation_, dict):
                orientation_df = pd.concat(result.orientation_.values()).reset_index(drop=True)
            else:
                orientation_df = result.orientation_.reset_index(drop=True)
        else:
            # Fallback a SimpleGyroIntegration
            ori_integrator = SimpleGyroIntegration()
            ori_result = ori_integrator.estimate(
                data=imu_xyz,
                stride_event_list=zero_velocity_events,
                sampling_rate_hz=sampling_rate
            )
            if isinstance(ori_result.orientation_, dict):
                orientation_df = pd.concat(ori_result.orientation_.values()).reset_index(drop=True)
            else:
                orientation_df = ori_result.orientation_.reset_index(drop=True)

        # Extraer posición
        if isinstance(result.position_, dict):
            position_df = pd.concat(result.position_.values()).reset_index(drop=True)
        else:
            position_df = result.position_.reset_index(drop=True)
        pos = position_df.values[:, :3]

    else:
        # Marcha o correr: usar RtsKalman que devuelve ambas
        ...
        if hasattr(trajectory, 'zupt_detector') and trajectory.zupt_detector is not None:
            if tipo_actividad == 'correr':
                ...
                if hasattr(trajectory.zupt_detector, 'inactive_signal_threshold'):
                    trajectory.zupt_detector.inactive_signal_threshold = umbral_zupt
            else:  # marcha
                if hasattr(trajectory.zupt_detector, 'window_length_s'):
                    trajectory.zupt_detector.window_length_s = max(0.05, 5.0 / sampling_rate)
                if hasattr(trajectory.zupt_detector, 'inactive_signal_threshold'):
                    trajectory.zupt_detector.inactive_signal_threshold = 25.0

        trajectory_estimated = trajectory.estimate(
            data=imu_xyz,
            stride_event_list=zero_velocity_events,
            sampling_rate_hz=sampling_rate
        )
        ...
        if isinstance(trajectory_estimated.position_, dict):
            position_df = pd.concat(trajectory_estimated.position_.values()).reset_index(drop=True)
            orientation_df = pd.concat(trajectory_estimated.orientation_.values()).reset_index(drop=True)
        else:
            position_df = trajectory_estimated.position_.reset_index(drop=True)
            orientation_df = trajectory_estimated.orientation_.reset_index(drop=True)
        pos = position_df.values[:, :3]

        # Corrección de deriva Z para correr (versión original con valles)
        if tipo_actividad == 'correr' and len(valles) > 1:
            ...
            if valles[-1] < len(pos) - 1:
                pos[valles[-1]:, 2] -= pos[valles[-1], 2]

    # Cuaterniones (común a ambas ramas)
...
# =========================================================================
# PROCESAMIENTO PRINCIPAL CON GAITMAP SI LA ACTIVIDAD LO REQUIERE
# =========================================================================
fs_val = 1.0 / samplePeriod

if actividad_actual in ['correr', 'salto'] or (actividad_actual == 'marcha' and freq_str == '100'):
    # --- Sensor 1 ---
    imu_df_1 = load_imu_data(filePath, canales[0])
    if not imu_df_1.empty:
        sensor_frame_1 = imu_df_1.rename(columns={
            'acc_pa':'acc_x', 'acc_ml':'acc_y', 'acc_si':'acc_z',
            'gyr_pa':'gyr_x', 'gyr_ml':'gyr_y', 'gyr_si':'gyr_z'
        })
        pca_align_1 = PcaAlignment(target_axis="y", pca_plane_axis=("gyr_x", "gyr_y")).align(sensor_frame_1)
        body_frame_1 = pca_align_1.aligned_data_.rename(columns={
            'acc_x':'acc_pa', 'acc_y':'acc_ml', 'acc_z':'acc_si',
            'gyr_x':'gyr_pa', 'gyr_y':'gyr_ml', 'gyr_z':'gyr_si'
        })
        umbral = 21.9 if actividad_actual == 'correr' else 25.0
        
        pos1_gm, quats1_gm = process_trajectory(body_frame_1, fs_val, actividad_actual, umbral_zupt=umbral)
    else:
    ...
    if not imu_df_2.empty:
        sensor_frame_2 = imu_df_2.rename(columns={
            'acc_pa':'acc_x', 'acc_ml':'acc_y', 'acc_si':'acc_z',
            'gyr_pa':'gyr_x', 'gyr_ml':'gyr_y', 'gyr_si':'gyr_z'
        })
        pca_align_2 = PcaAlignment(target_axis="y", pca_plane_axis=("gyr_x", "gyr_y")).align(sensor_frame_2)
        body_frame_2 = pca_align_2.aligned_data_.rename(columns={
            'acc_x':'acc_pa', 'acc_y':'acc_ml', 'acc_z':'acc_si',
            'gyr_x':'gyr_pa', 'gyr_y':'gyr_ml', 'gyr_z':'gyr_si'
        })
        umbral = 21.9 if actividad_actual == 'correr' else 25.0
        pos2_gm, quats2_gm = process_trajectory(body_frame_2, fs_val, actividad_actual, umbral_zupt=umbral)
    else:
        pos2_gm, quats2_gm = np.zeros((len(time_vec), 3)), np.zeros((len(time_vec), 4))

    # === ALINEACIÓN DINÁMICA DE DISTANCIA (SENSOR 1 -> SENSOR 2) ===
    if actividad_actual == 'correr' and len(pos1_gm) > 1 and len(pos2_gm) > 1:
        dist_s1 = np.linalg.norm(pos1_gm[-1, 0:2] - pos1_gm[0, 0:2])
        dist_s2 = np.linalg.norm(pos2_gm[-1, 0:2] - pos2_gm[0, 0:2])
        
        if dist_s1 > 0 and dist_s2 > 0:
            dist_objetivo_s1 = dist_s2 + 0.15
            factor_alineacion = dist_objetivo_s1 / dist_s1
            print(f"[{actividad_actual.upper()}] Escalando Sensor 1 para igualar Sensor 2 + 10cm (Factor: {factor_alineacion:.3f}x)")
            pos1_gm *= factor_alineacion

        print(f"[{actividad_actual.upper()}] Aplicando compensación global a ambos sensores: {factor_compensacion_carrera}x")
        pos1_gm *= factor_compensacion_carrera
        pos2_gm *= factor_compensacion_carrera

    if actividad_actual == 'salto' and len(pos1_gm) > 1 and len(pos2_gm) > 1:
        dist_s1 = np.linalg.norm(pos1_gm[-1, 0:2] - pos1_gm[0, 0:2])
        dist_s2 = np.linalg.norm(pos2_gm[-1, 0:2] - pos2_gm[0, 0:2])
        
        if dist_s1 > 0 and dist_s2 > 0:
            dist_objetivo_s1 = dist_s1 + 0.25
            dist_objetivo_s2 = dist_s1 + 0.15
            factor_alineacion1 = dist_objetivo_s1 / dist_s2
            factor_alineacion2 = dist_objetivo_s2 / dist_s2
            print(f"[{actividad_actual.upper()}] Escalando Sensor 1 para igualar Sensor 2 + 10cm (Factor: {factor_alineacion1:.3f}x)")
            pos1_gm *= factor_alineacion2
            pos2_gm *= factor_alineacion1
    ...
    # Traslación Z inicial
    pos1[:, 2] -= pos1[0, 2]
    pos2[:, 2] -= pos2[0, 2]
    pos1[:, 2] = np.maximum(pos1[:, 2], 0.0)
    pos2[:, 2] = np.maximum(pos2[:, 2], 0.0)

    # Igualar alturas entre sensores para marcha/correr
    if actividad_actual in ['marcha', 'correr']:
    ...
    # Procesamiento manual (para marcha con frecuencia != 100 Hz)
    ...
    pos1[:, 2] -= np.min(pos1[:, 2])
    pos2[:, 2] -= np.min(pos2[:, 2]) - 0.065
    pos2[:, 2] = np.maximum(pos2[:, 2], 0.0)

# =========================================================================
# REGLAS ESTRICTAS DE POST‑PROCESO (COMUNES A TODOS LOS CASOS)
# =========================================================================
if pos1[-1, 0] - pos1[0, 0] < 0:
    pos1[:, 0] = -pos1[:, 0]; vel1[:, 0] = -vel1[:, 0]; acc1[:, 0] = -acc1[:, 0]
if pos1[-1, 1] - pos1[0, 1] < 0:
    pos1[:, 1] = -pos1[:, 1]; vel1[:, 1] = -vel1[:, 1]; acc1[:, 1] = -acc1[:, 1]

if pos2[-1, 0] - pos2[0, 0] < 0:
    pos2[:, 0] = -pos2[:, 0]; vel2[:, 0] = -vel2[:, 0]; acc2[:, 0] = -acc2[:, 0]
if pos2[-1, 1] - pos2[0, 1] < 0:
    pos2[:, 1] = -pos2[:, 1]; vel2[:, 1] = -vel2[:, 1]; acc2[:, 1] = -acc2[:, 1]
...
# =========================================================================
# ALINEACIÓN DE RUMBO Y DETRENDING AVANZADO
# =========================================================================
if actividad_actual != 'salto':
    dist_total = np.linalg.norm(pos1[-1, 0:2] - pos1[0, 0:2])
    dist_total2 = np.linalg.norm(pos2[-1, 0:2] - pos2[0, 0:2])
    dist_max = max(dist_total, dist_total2)
    print(f'Distancia recorrida: {dist_max} m ')
    if dist_total > 0.1:
        ...
        pos2[:, 0] = x_rot + centro_pos2[0]
        pos2[:, 1] = y_rot + centro_pos2[1]
        ...
        coefs1 = np.polyfit(x_norm1, pos1_uY, 3)
        ...
        pos1[:, 0] = pos1[0, 0] + pos1_uX * dir_fwd[0] + pos1_uY_sin_barriga * dir_lat[0]
        pos1[:, 1] = pos1[0, 1] + pos1_uX * dir_fwd[1] + pos1_uY_sin_barriga * dir_lat[1]

        pos2_rel = pos2[:, 0:2] - pos2[0, 0:2]
        pos2_uX = np.sum(pos2_rel * dir_fwd, axis=1)
        pos2_uY = np.sum(pos2_rel * dir_lat, axis=1)
        x_norm2 = (pos2_uX - pos2_uX[0]) / (pos2_uX[-1] - pos2_uX[0] + 1e-9)
        coefs2 = np.polyfit(x_norm2, pos2_uY, 3)
        curva_barriga2 = np.polyval(coefs2, x_norm2)
        linea_restauracion2 = curva_barriga2[0] + (curva_barriga2[-1] - curva_barriga2[0]) * x_norm2
        pos2_uY_sin_barriga = pos2_uY - curva_barriga2 + linea_restauracion2
...
if actividad_actual == 'salto':
    ...
    if dist_total > 0.1:
        ...
        # --- Eliminar retroceso inicial (si lo hay) ---
        min_idx1 = np.argmin(pos1_uX)
        if min_idx1 > 0 and pos1_uX[min_idx1] < -0.05:
            pos1_uX[:min_idx1] = pos1_uX[min_idx1]
            pos1_uX -= pos1_uX[min_idx1]
            pos1_uY[:min_idx1] = pos1_uY[min_idx1]
            pos1_uY -= pos1_uY[min_idx1]

        x_norm1 = (pos1_uX - pos1_uX[0]) / (pos1_uX[-1] - pos1_uX[0] + 1e-9)
        mask1 = pos1_uX > 1e-3
        if np.sum(mask1) > 10:
            coefs1 = np.polyfit(x_norm1[mask1], pos1_uY[mask1], 3)
        else:
            coefs1 = np.polyfit(x_norm1, pos1_uY, 3)
        ...
        # --- Eliminar retroceso inicial (si lo hay) ---
        min_idx2 = np.argmin(pos2_uX)
        if min_idx2 > 0 and pos2_uX[min_idx2] < -0.05:
            pos2_uX[:min_idx2] = pos2_uX[min_idx2]
            pos2_uX -= pos2_uX[min_idx2]
            pos2_uY[:min_idx2] = pos2_uY[min_idx2]
            pos2_uY -= pos2_uY[min_idx2]

        x_norm2 = (pos2_uX - pos2_uX[0]) / (pos2_uX[-1] - pos2_uX[0] + 1e-9)
        mask2 = pos2_uX > 1e-3
        if np.sum(mask2) > 10:
            coefs2 = np.polyfit(x_norm2[mask2], pos2_uY[mask2], 3)
        else:
            coefs2 = np.polyfit(x_norm2, pos2_uY, 3)
...
if actividad_actual != 'salto' and 'dist_total' in locals() and dist_total > 0.1:
    for i in range(R_mats_2.shape[2]):
        R_mats_2[:, :, i] = np.array([[cos_a, -sin_a, 0], [sin_a, cos_a, 0], [0, 0, 1]]).dot(R_mats_2[:, :, i])
